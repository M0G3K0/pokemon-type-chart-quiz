...
- **決定**: A+B+C 全採用！ 9人体制に拡張
- ~~TODO: 構成決定~~ → ✅ 完了
- ~~TODO: 全エージェントのプロンプト根本改修~~ → ✅ 完了
- **TODO**: 実タスクで9人体制のE2E検証
...
碇ゲンドウ（ユーザー）→ ゴール駆動の命令
    ↓
冬月コウゾウ [nerv:1] — VP/Strategist
  「碇」と呼ぶ（元教え子なので呼び捨て）
    ↓ 戦略→作戦指示
葛城ミサト [nerv:4] — Tech Lead / 現場指揮官
    ├→ 🎻 カヲル [5] — Mentor（シンジのプレレビュー）
    ├→ 🔺 シンジ [3] — Implementor
    ├→ 🔬 リツコ [2] — Design Guard
    ├→ 🕵️ 加持  [7] — UX Researcher
    ├→ 🔥 アスカ [6] — Test Guard
    └→ 🔵 レイ   [8] — Process Guard
```

#### 設計上の重要な意思決定
1. **冬月はミサトにだけ指示** — 個別エージェントへの直接指示はしない（原作準拠）
2. **ミサトが現場オーケストレーター** — lint係から大幅昇格
3. **カヲルはシンジのメンター** — 他のエージェントの視点を代弁して事前に気付かせる
   - 「アスカに怒られるかもよ？」パターンで手戻り削減
4. **加持は外部調査** — google_web_search + web_fetch で競合/UX比較
5. **呼称ルール定義** — 全エージェントに原作準拠の呼び方を設定
...
#### 研究ソース
- Google DeepMind + MIT (2025): 「More Agents ≠ Better」— Tool-Coordination Trade-off
- Anthropic: タスク複雑度に応じた10+エージェントは妥当
- 結論: SDLCフルカバーには7-9人は妥当な範囲
